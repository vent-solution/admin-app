  ...
  return (
    <tr className="cursor-pointer text-sm text-center border-y-2 hover:bg-gray-100">
      ...
      <td className="px-2 text-blue-700">
        <Link to={`/facilies/${bid.facility.facilityId}`}>
      ...
      {!bid.facility.landlord?.companyName && (
        <td className="px-2">
          {bid.facility.landlord?.user?.firstName +
            " " +
            bid.facility.landlord?.user?.lastName}
        </td>
      )}

      {bid.facility.landlord?.companyName && (
        <td className="px-2">{bid.facility.landlord?.companyName}</td>
      )}
